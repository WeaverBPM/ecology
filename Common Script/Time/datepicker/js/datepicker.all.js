...
$(function () {
  ...
  var PICKERHEADERTPL = '<div class="{{className}}__header">' +
    '{{prev}}' +
    '<span role="button" class="{{className}}__header-label {{className}}__header-year"><span>{{year}}</span></span>' +
    '<span role="button" class="{{className}}__header-label {{className}}__header-month"><span>{{month}}</span></span>' +
    '{{next}}' +
    '</div>';
  ...
  var RANGEPICKERMAINTPL = '<div class="c-datepicker-picker c-datepicker-date-range-picker c-datepicker-popper {{hasTime}} {{hasSidebar}}" x-placement="top-start">' +
    ...
    '</div>' +
    '</div>' +
    PICKERFOOTERTPL.replace(/{{className}}/g, 'c-datepicker-picker__btn-clear').replace(/{{text}}/g, 'Clear') +
    PICKERARROWTPL +
    '</div>';
  var PICKERFOOTERNOWBUTTON = PICKERFOOTERTPL.replace(/{{className}}/g, 'c-datepicker-picker__btn-now').replace(/{{text}}/g, 'Now');
  var PICKERFOOTERCLEARBUTTON = PICKERFOOTERTPL.replace(/{{className}}/g, 'c-datepicker-picker__btn-clear').replace(/{{text}}/g, 'Clear');
  // 单个
  var DATEPICKERMAINTPL = '<div class="c-datepicker-picker c-datepicker-date-picker c-datepicker-popper {{hasTime}} {{hasSidebar}}" x-placement="top-start">' +
    ...
    '</div>' +
    '<div class="c-datepicker-date-picker__left">' +
  ...
  $.extend(Year.prototype, {
    ...
    renderHtml: function (year) {
      ...
      this.picker.$container.find('.c-datepicker-date-picker__header-year span').text(min + '-' + (min + 9));
  ...
  function SingleDatePicker(datePickerObject) {
    ...
    this.init(datePickerObject.$target);
  }
  ...
  $.extend(SingleDatePicker.prototype, {
    ...
    initShow: function (ele) {
      ...
      $(ele).find(".c-datePicker__input-time").clockTimePicker({});
    },
    event: function () {
      if (this.hasTime) {
        this.eventHasTime();
      ...
      this.$container.on('click', '.c-datepicker-picker__link-btn.confirm', function () {
        ...
        if (!_this.$input.val()) {
          ...
          setValue(_this, _this.$container.find(".c-datePicker__input-day").val() + " " + _this.$container.find(".c-datePicker__input-time").val());
        }
        _this.datePickerObject.hide();
      });
    },
  ...
  $.extend(RangeDatePicker.prototype, {
    ...
    initShow: function () {
      ...
      // 初始化年月日十分秒panel
      DATEPICKERAPI.initShowObject(this, dataFormat);
      $(".c-datepicker-picker__time").clockTimePicker({});
    },
    // 默认值回填时间插件选择框
